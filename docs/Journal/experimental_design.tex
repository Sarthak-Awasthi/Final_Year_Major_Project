...
\begin{tabular}{@{}lcc@{}}
\toprule
Condition & Final Cooperation $\cK$ & $\Delta$ vs C1 \\
\midrule
\textbf{C1}: Full System              & 0.851 & --- \\
\textbf{C3}: No RL (Schedules Only)   & 0.500 & $-0.351$ \\
\textbf{C4}: Flat MDP                 & 0.953 & $+0.102$ \\
\textbf{C5}: No Shocks                & 0.873 & $+0.022$ \\
\textbf{C6}: No Role Masking          & 0.763 & $-0.088$ \\
\textbf{C7}: Static $\lambda = 0.325$ & 0.693 & $-0.158$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Flat MDP achieves higher cooperation than hierarchical:} C4 ($\cK = 0.953$) exceeds C1 by $+0.102$. Faster quest cycling in the flat structure produces more learning episodes within 500 turns.
    ...
    \item \textbf{Dynamic $\lambda$ amplifies cooperation by $+0.158$:} C7 (static $\lambda = 0.325$, $\cK = 0.693$) still shows emergence from the community reward signal, but the dynamic feedback loop (C1, $\cK = 0.851$) provides a substantial boost --- the second-largest single-component contribution after RL itself.
\end{itemize}
